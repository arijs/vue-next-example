...
function loadCompList(comps, getLoader, onFinish) {
	...
	var all = allCallback({
		...
		onDone: function(err, result) {
			var ref = this._current;
			if (!ref) return;
			ref.err = err;
			ref.result = result;
			if (err) this.error.push(ref);
			else this.success.push(ref);
			this.pending[ref.index] = null;
		},
		onFinish: onFinish
		// function(state) {
		// 	console.log('Page/home subcomponents loaded', state);
		// 	pageLoading.value = false;
		// }
	});
	var allStart = all();
	forEach(comps, function(cname) {
		cname = cname.replace(/\/+/g, '--');
		var done = all(cname);
		// var loader = global.resolveUserCompLoader(cname);
		var loader = getLoader(cname);
		if (loader) {
